...
export class NgxIntlTelInputComponent implements OnInit, OnChanges {
  ...
  */
        ...
        setTimeout(() => {
          if (this.countriesList.includes(this.selectedCountry)) {
            ...
            const highlightCountry = document.getElementById(
              'iti-0__item-' + this.selectedCountry.iso2
            ) as HTMLElement;
            this.countryHighlight(highlightCountry);
          }
        }, 10);
  ...
  public searchCountry(event) {
    var key = event.key;
    const disallowedKeys = ['ArrowDown', 'ArrowUp', 'Enter'];
    ...
    const countrySearchTextFormatted = this.countrySearchText
      .toLowerCase()
      .replace(/[+]/g, '');
    // @ts-ignore
    const countries = this.allCountries
      .filter((c) => {
        if (this.searchCountryField.indexOf(SearchCountryField.All) > -1) {
          ...
          if (c.iso2.toLowerCase().indexOf(countrySearchTextFormatted) > -1) {
            return c;
          }
          if (c.name.toLowerCase().indexOf(countrySearchTextFormatted) > -1) {
            return c;
          }
          if (c.dialCode.indexOf(countrySearchTextFormatted) > -1) {
            return c;
          }
        } else {
          ...
          if (
            this.searchCountryField.indexOf(SearchCountryField.DialCode) > -1
          ) {
            if (c.dialCode.indexOf(countrySearchTextFormatted) > -1) {
              return c;
            }
      ...
      .sort((a, b) => {
        if (a.name.toLowerCase().indexOf(countrySearchTextFormatted) > -1) {
          if (
            b.name.toLowerCase().indexOf(countrySearchTextFormatted) <
            a.name.toLowerCase().indexOf(countrySearchTextFormatted)
          ) {
            return 1;
          }
          if (
        ...
        if (a.iso2.toLowerCase().indexOf(countrySearchTextFormatted) > -1) {
          if (
            b.iso2.toLowerCase().indexOf(countrySearchTextFormatted) <
            a.iso2.toLowerCase().indexOf(countrySearchTextFormatted)
          ) {
            return 1;
          }
          if (
        ...
        if (a.dialCode.indexOf(countrySearchTextFormatted) > -1) {
          if (
            b.dialCode.indexOf(countrySearchTextFormatted) <
            a.dialCode.indexOf(countrySearchTextFormatted)
          ) {
            return 1;
          }
          if (
  ...
  public onArrowKeyPress(event: KeyboardEvent): void {
    event.preventDefault();
    ...
    var key = event.key;
    if (key === 'ArrowDown') {
      if (this.countryElementIndex + 1 < countryElementList.length) {
        this.countryElementIndex += 1;
        this.countryHighlight(countryElementList[this.countryElementIndex]);
      }
    } else {
      if (this.countryElementIndex > 0) {
        this.countryElementIndex -= 1;
        this.countryHighlight(countryElementList[this.countryElementIndex]);
      }
    }
  ...
  public countryHighlight(element: HTMLElement): void {
    const countryElementList = Array.from(
  ...
  public onEnterKeyPress(event: KeyboardEvent): void {
    ...
    this.selectedCountry = this.allCountries.filter(
      (country) =>
        country.name ===
        countryElementList[this.countryElementIndex]?.getElementsByClassName(
          'iti__country-name'
        )[0].innerHTML
    )[0];
